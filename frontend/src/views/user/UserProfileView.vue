<template>
  <div v-if="loading" class="text-center py-10 text-gray-700 dark:text-gray-300">Loading profile...</div>
  <div v-else-if="error" class="text-center py-10 text-red-500 dark:text-red-400">Error loading profile.</div>
  <div v-else-if="profile" class="container mx-auto px-4 py-8">
    ...
    <div class="flex flex-col md:flex-row items-center md:items-start mb-8">
      ...
      <div class="text-center md:text-left w-full flex-grow">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2">
          <h1 class="text-3xl font-bold text-gray-800 dark:text-gray-200 mb-2 sm:mb-0 flex items-center">
            {{ profile.username }}
            <span v-if="profile.is_verified" class="ml-2" title="Verified Subscriber">
              <svg class="w-5 h-5 text-blue-500 dark:text-blue-300 inline" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div class="mb-12">
      ...
      <div class="mt-6">
        ...
        <template v-if="activeTab === 'bookmarks'">
          <div class="flex items-center gap-1 py-1 px-1 rounded-md bg-gray-50 dark:bg-gray-800 w-full sm:w-auto mb-4">
            <span class="text-sm font-medium text-gray-600 dark:text-gray-400 hidden sm:inline pl-2">Filter:</span>
            <OrientationFilter />
          </div>
          <BookmarkGrid
            :fetchBookmarks="fetchUserBookmarks"
            :filters="mergedBookmarkFilters"
            :showPagination="true"
          ...
          <div v-if="!selectedCollection && !selectedChannel">
            ...
            <div v-else-if="collections.length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              <div v-for="collection in collections" :key="collection.id"
                class="bg-white  rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer overflow-hidden group block"
                ...
                <div class="w-full h-32 bg-gray-200  flex items-center justify-center overflow-hidden">
                  ...
                  <span v-else class="text-3xl font-semibold text-gray-500 ">{{ collection.name.charAt(0).toUpperCase() }}</span>
                </div>
                <div class="p-4">
                  <h3 class="text-lg font-semibold text-gray-800  truncate group-hover:text-indigo-600 ">{{ collection.name }}</h3>
                  <p class="text-sm mt-1 truncate h-10" :class="collection.description ? 'text-gray-600 ' : 'text-gray-400 '">
          ...
          <div v-else-if="selectedCollection && !selectedChannel">
            ...
            <div v-if="selectedCollection.channels?.length" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              <div v-for="channel in selectedCollection.channels" :key="channel.id"
                ...
                <div class="w-full h-24 bg-gray-200  flex items-center justify-center overflow-hidden">
                  ...
                  <span v-else class="text-2xl font-semibold text-gray-500 ">{{ channel.name.charAt(0).toUpperCase() }}</span>
                </div>
                <div class="p-4">
                  <h3 class="text-base font-semibold text-gray-800  truncate group-hover:text-indigo-600 ">{{ channel.name }}</h3>
                  <p class="text-sm mt-1 truncate h-8" :class="channel.description ? 'text-gray-600 ' : 'text-gray-400 '">
    ...
    <BaseModal v-model="showEditModal">
      <h2 class="text-lg font-semibold mb-4 text-gray-800 dark:text-gray-200">Edit Profile</h2>
      <form @submit.prevent="saveProfileEdits">
        <div class="mb-4">
          ...
          <input type="file" accept="image/*" @change="onAvatarChange" class="block w-full text-sm text-gray-700 dark:text-gray-300" />
          <div v-if="avatarPreview" class="mt-2">
            <img :src="avatarPreview" alt="Avatar Preview" class="h-20 w-20 rounded-full object-cover border dark:border-gray-600" />
          </div>
        </div>
        <div class="mb-4">
          <label class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Bio</label>
          <textarea v-model="editableProfile.bio" class="w-full px-3 py-2 border rounded dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200"></textarea>
        </div>
        <div class="mb-4">
          <label class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Default Bookmark Orientation</label>
          <select v-model="editableProfile.default_bookmark_orientation" class="w-full px-3 py-2 border rounded dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200">
        ...
        <div class="mb-4">
          ...
          <select v-model="editableProfile.default_bookmark_collection" class="w-full px-3 py-2 border rounded dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200">
            <option value="">None</option>
        ...
        <div class="mb-4">
          ...
          <div class="flex flex-col gap-2">
            <label class="inline-flex items-center"><input type="checkbox" v-model="editableProfile.notify_on_follow" class="mr-2 dark:bg-gray-800 dark:border-gray-600" />Notify on Follow</label>
            <label class="inline-flex items-center"><input type="checkbox" v-model="editableProfile.notify_on_own_video_bookmarked" class="mr-2 dark:bg-gray-800 dark:border-gray-600" />Notify on Own Video Bookmarked</label>
            <label class="inline-flex items-center"><input type="checkbox" v-model="editableProfile.notify_on_new_bookmark_from_followed_user" class="mr-2 dark:bg-gray-800 dark:border-gray-600" />Notify on New Bookmark from Followed User</label>
            <label class="inline-flex items-center"><input type="checkbox" v-model="editableProfile.notify_on_own_video_liked" class="mr-2 dark:bg-gray-800 dark:border-gray-600" />Notify on Own Video Liked</label>
          </div>
        </div>
        <div class="flex justify-end">
          <button type="button" class="mr-2 px-4 py-2 rounded bg-gray-200 text-gray-700 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600" @click="showEditModal = false">Cancel</button>
          <button type="submit" class="px-4 py-2 rounded bg-indigo-600 text-white hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600">Save</button>
        </div>
      </form>
    </BaseModal>
  </div>
...
const editableProfile = ref({
  ...
  notify_on_own_video_bookmarked: true,
  notify_on_new_bookmark_from_followed_user: true,
  notify_on_own_video_liked: true,
})
...
const fetchUserBookmarks = async (params = {}) => {
  return await fetchVideos({ ...params, user: username.value, page: currentPage.value })
}
const fetchUserLiked = async (params = {}) => {
  return await fetchVideos({ ...params, liked_by: username.value })
}
